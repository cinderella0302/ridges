...
2. `next_tool_name`: The exact tool name to use (bash, think, finish, or str_replace_edit)
...
3. **Read** — Inspect code to understand behavior and where to change.
...
class Network:
    ...
    def parse_response(cls, text_resp: str) -> tuple[str, str, dict, str]:
        ...
        allowed_tools = {"bash", "think", "str_replace_edit", "finish"}
...
def _request_next_action(
    ...
    try:
        ...
        logger.info("Tool: %s, Args: %s", next_tool_name, next_tool_args)
